...
{
    // Create shader
    if (!createShaderByName(constant::glsl_shader_name::count_ray_buffer_state,
            m_cs_count_ray_buffer_state.shader))
        return false;
    ...
    // Create SSBOs
    m_cs_count_ray_buffer_state.ssbo_output =
        m_context->ogl->createShaderStorageBuffer(sizeof(int32_t) * 2);
    m_cs_count_ray_buffer_state.ssbo_temp =
        m_context->ogl->createShaderStorageBuffer(sizeof(int32_t) * 2 *
            m_context->parameters.number_rays_per_buffer);
    if (m_cs_count_ray_buffer_state.ssbo_output == nullptr ||
        m_cs_count_ray_buffer_state.ssbo_temp == nullptr)
        return false;

    // Compute work group size
    m_cs_count_ray_buffer_state.num_work_groups = 
        ComputeHelper::getNumWorkGroups(
            m_context->parameters.number_rays_per_buffer,
            constant::glsl_local_group_size::count_ray_buffer_state);
    ...
    m_cs_count_ray_buffer_state.uniform_num_particles =
        m_cs_count_ray_buffer_state.shader->getUniformLocation("num_particles");
    m_cs_count_ray_buffer_state.uniform_num_rays_per_buffer =
        m_cs_count_ray_buffer_state.shader->getUniformLocation("num_rays_per_buffer");

    m_cs_count_ray_buffer_state.shader->use();
    glUniform1ui(m_cs_count_ray_buffer_state.uniform_num_particles,
        static_cast<GLuint>(m_context->particles.size()));
    glUniform1ui(m_cs_count_ray_buffer_state.uniform_num_rays_per_buffer,
...
{
    // Bind shader
    LDPLAB_PROFILING_START(update_ray_buffer_state_shader_binding);
    m_cs_count_ray_buffer_state.shader->use();
    LDPLAB_PROFILING_STOP(update_ray_buffer_state_shader_binding);

    // Bind SSBOs
    LDPLAB_PROFILING_START(update_ray_buffer_state_ssbo_binding);
    buffer.ssbo.particle_index->bindToIndex(0);
    m_cs_count_ray_buffer_state.ssbo_temp->bindToIndex(1);
    m_cs_count_ray_buffer_state.ssbo_output->bindToIndex(2);
    LDPLAB_PROFILING_STOP(update_ray_buffer_state_ssbo_binding);

    // Execute shader
    LDPLAB_PROFILING_START(update_ray_buffer_state_shader_execution);
    m_cs_count_ray_buffer_state.shader->execute(
        m_cs_count_ray_buffer_state.num_work_groups);
    LDPLAB_PROFILING_STOP(update_ray_buffer_state_shader_execution);

    // Download data
    LDPLAB_PROFILING_START(update_ray_buffer_state_data_download);
    int32_t output_data[2] = { 0 };
    m_cs_count_ray_buffer_state.ssbo_output->download(output_data);
