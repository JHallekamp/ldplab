...
ldplab::rtsogl::Pipeline::Pipeline(
...
bool ldplab::rtsogl::Pipeline::initShaders()
{
...
void ldplab::rtsogl::Pipeline::setup()
{
...
void ldplab::rtsogl::Pipeline::finalizeOutput(RayTracingStepOutput& output)
{
...
void ldplab::rtsogl::Pipeline::execute(size_t job_id, size_t batch_size)
{
    LDPLAB_ASSERT(job_id < m_buffer_controls.size());
    LDPLAB_LOG_DEBUG("RTSOGL context %i: Ray tracing pipeline executes "\
        "pipeline instance %i",
    ...
    do
    {
        ...
        LDPLAB_LOG_TRACE("RTSOGL context %i: Filled batch buffer %i "\
            "with %i initial rays",
            m_context.uid, 
        ...
        {
            LDPLAB_LOG_TRACE("RTSOGL context %i: Pipeline instance "\
            ...
            LDPLAB_LOG_TRACE("RTSOGL context %i: Pipeline instance %i "\
    ...
    LDPLAB_LOG_DEBUG("RTSOGL context %i: Pipeline instance %i executed "\
        "successfully in a total of %i batches",
        m_context.uid, job_id, num_batches);
}

void ldplab::rtsogl::Pipeline::processBatch(
    RayBuffer& buffer, BufferControl& buffer_control)
{
        ...
        m_context.flags.emit_warning_on_maximum_branching_depth_discardment)
    {
        ...
        for (size_t i = 0; i < buffer.size; ++i)
        {
            ...
            real_t intensity = buffer.ray_properties_data[i].intensity;
            avg_intensity += intensity;
            if (intensity > max_intensity)
                max_intensity = intensity;
        }
        avg_intensity /= static_cast<real_t>(buffer.active_rays);
        LDPLAB_LOG_WARNING("RTSOGL context %i: Pipeline reached max "\
