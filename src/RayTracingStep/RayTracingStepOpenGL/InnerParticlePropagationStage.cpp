...
ldplab::rtsogl::LinearIndexGradientRodParticlePropagation::
...
bool ldplab::rtsogl::LinearIndexGradientRodParticlePropagation::initShaders()
{
    // Create shader
...
void ldplab::rtsogl::LinearIndexGradientRodParticlePropagation::execute(
    RayBuffer& rays,
    IntersectionBuffer& intersection,
    OutputBuffer& output)
{
    LDPLAB_LOG_TRACE("RTSOGL context %i: Execute inner particle ray "\
    ...
    LDPLAB_LOG_TRACE("RTSOGL context %i: Inner particle ray propagation on "\
        "buffer %i completed",
        m_context.uid,
        rays.uid);
}
