...
ldplab::rtsogl::InitialStageBoundingSpheresHomogenousLight::
    InitialStageBoundingSpheresHomogenousLight(
        Context& context)
    :
    m_context{ context },
    m_bounding_spheres{ ((BoundingSphereData*)
        context.bounding_volume_data.get())->sphere_properties_data },
    ...
    m_rasterization_step_size{ 0 }
{
    LDPLAB_LOG_INFO("RTSOGL context %i: "\
        ...
        m_context.uid);
    //LDPLAB_LOG_INFO("RTSOGL context %i: Created initial stage for bounding"\
    //    " spheres and homogenous light sources", m_context.uid);
}

void ldplab::rtsogl::InitialStageBoundingSpheresHomogenousLight::setup()
{
    LDPLAB_LOG_DEBUG("RTSOGL context %i: Project particles on light sources",
        m_context.uid);
    ...
    for (size_t i = 0; i < m_context.light_sources.size(); ++i)
    {
        ...
        for (size_t j = 0; j < m_context.particles.size(); ++j)
        {
            ...
            if (t <= m_bounding_spheres[j].radius)
            {
                LDPLAB_LOG_WARNING("RTSOGL context %i: Particle "\
            ...
            LDPLAB_LOG_DEBUG("RTSOGL context %i: Particle %i is projected"\
                " onto light source %i",
                m_context.uid, j, i);
            ...
            proj.center = center;
            proj.radius = radius;
            proj.depth = glm::length(wrldctr - m_bounding_spheres[j].center);
...
void ldplab::rtsogl::InitialStageBoundingSpheresHomogenousLight::
...
inline void ldplab::rtsogl::InitialStageBoundingSpheresHomogenousLight::
...
bool ldplab::rtsogl::InitialStageBoundingSpheresHomogenousLight::createBatch(
    RayBuffer& initial_batch_buffer)
{
    std::lock_guard<std::mutex> lock(m_mutex);

    LDPLAB_LOG_TRACE("RTSOGL context %i: Create initial batch rays for"\
        " batch buffer %i",
        m_context.uid,
    ...
    for (size_t i = 0; i < initial_batch_buffer.size; ++i)
        initial_batch_buffer.particle_index_data[i] = -1;
                ...
                /* number of rays is increased during ray creation */)
            {
                ...
                if (hasToCreateRay(projection, light))
                {
                        ...
                        m_rasterization_y <= light.vertical_size)
                    {
                        // Create ray
                        // Set ray origin
                        initial_batch_buffer.ray_properties_data[nr].origin =
                            light.origin_corner
                        ...
                        initial_batch_buffer.ray_properties_data[nr].
                            min_bounding_volume_distance =
                            projection.depth;

                        // Set initial ray particle index
                        initial_batch_buffer.particle_index_data[nr] = 
