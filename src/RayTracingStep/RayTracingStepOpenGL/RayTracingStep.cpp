...
ldplab::rtsogl::ParticleTransformationData::Transformation
    buildTranformationData(ldplab::Mat3 rotation_scale, ldplab::Vec3 translation)
{
    ldplab::rtsogl::ParticleTransformationData::Transformation trsf;
...
ldplab::rtsogl::RayTracingStep::RayTracingStep(
...
void ldplab::rtsogl::RayTracingStep::updateContext(
    const SimulationState& input)
{
    ...
    for (size_t i = 0; i < m_context->particles.size(); ++i)
    {
        ...
        {
            LDPLAB_LOG_ERROR("RTSOGL context %i: Could not update particle "\
                "transformations, particle %i is not present in the given "\
                "simulation state, abort RTSOGL execution",
                m_context->uid,
                puid);
            return;
        }
...
bool ldplab::rtsogl::RayTracingStep::initGPU(
    const RayTracingStepOpenGLInfo& info)
{
    ...
    if (m_context->particle_data->type() == IParticleData::Type::rod_particles)
    {
        ...
        {
            LDPLAB_LOG_ERROR("RTSOGL context %i: Failed to create "\
    ...
    {
        LDPLAB_LOG_ERROR("RTSOGL context %i: Could not create "\
...
ldplab::Mat3 ldplab::rtsogl::RayTracingStep::getRotationMatrix(
    real_t rx,
    real_t ry,
    real_t rz,
    RotationOrder order)
{
    ...
    LDPLAB_LOG_WARNING("RTSOGL context %i: Encountered unknown "\
        "rotation order, assumes xyz instead.");
    return rotz * roty * rotx;
}

void ldplab::rtsogl::RayTracingStep::execute(
    const SimulationState& input, RayTracingStepOutput& output)
{
    ...
    LDPLAB_PROFILING_START(rtsogl_update_context);
    updateContext(input);
    LDPLAB_PROFILING_STOP(rtsogl_update_context);

    // Execute pipeline
    LDPLAB_LOG_DEBUG("RTSOGL context %i: Setup ray tracing pipeline",
        m_context->uid);
    LDPLAB_PROFILING_START(rtsogl_setup_pipeline);
    m_context->pipeline->setup();
    LDPLAB_PROFILING_STOP(rtsogl_setup_pipeline);

    LDPLAB_LOG_DEBUG("RTSOGL context %i: Execute ray tracing pipeline",
        m_context->uid);
    LDPLAB_PROFILING_START(rtsogl_execute_pipeline);
    ...
    LDPLAB_PROFILING_STOP(rtsogl_execute_pipeline);

    std::chrono::steady_clock::time_point end =
        std::chrono::steady_clock::now();
    const real_t elapsed_time = std::chrono::duration<real_t>(
            end - start).count();
    LDPLAB_LOG_INFO("RTSOGL context %i: Ray tracing step executed after %fs",
