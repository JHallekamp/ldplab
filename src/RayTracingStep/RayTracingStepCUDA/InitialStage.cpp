...
ldplab::rtscuda::InitialStageBoundingSpheresHomogenousLight::
...
void ldplab::rtscuda::InitialStageBoundingSpheresHomogenousLight::setup()
{
    LDPLAB_LOG_DEBUG("RTSCUDA context %i: Project particles on light sources",
        m_context.uid);
    ...
    for (size_t i = 0; i < m_context.light_sources.size(); ++i)
    {
        ...
        for (size_t j = 0; j < m_context.particles.size(); ++j)
        {
            ...
            {
                LDPLAB_LOG_TRACE("RTSCUDA context %i: Particle %i has no"\
            ...
            {
                LDPLAB_LOG_WARNING("RTSCUDA context %i: Particle "\
            ...
            LDPLAB_LOG_DEBUG("RTSCUDA context %i: Particle %i is projected"\
                " onto light source %i",
                m_context.uid, j, i);
...
void ldplab::rtscuda::InitialStageBoundingSpheresHomogenousLight::
...
inline void ldplab::rtscuda::InitialStageBoundingSpheresHomogenousLight::
...
bool ldplab::rtscuda::InitialStageBoundingSpheresHomogenousLight::createBatch(
    RayBuffer& initial_batch_buffer)
{
    std::lock_guard<std::mutex> lock(m_mutex);

    LDPLAB_LOG_TRACE("RTSCUDA context %i: Create initial batch rays for"\
        " batch buffer %i",
        m_context.uid,
