...
ldplab::rtscuda::RayTracingStep::RayTracingStep(
    ...
    RotationOrder order)
{
    ...
    LDPLAB_LOG_WARNING("RTSCUDA context %i: Encountered unknown rotation "\
        "order, assumes xyz instead.");
    return rotz * roty * rotx;
}

void ldplab::rtscuda::RayTracingStep::updateContext(const SimulationState& input)
{
    for (size_t i = 0; i < m_context->particles.size(); ++i)
    {
        ...
        {
            LDPLAB_LOG_ERROR("RTSCUDA context %i: Could not update particle "\
                "transformations, particle %i is not present in the given "\
                "simulation state, abort RTSCUDA execution",
                m_context->uid,
                puid);
            return;
        }
    ...
    const SimulationState& input, RayTracingStepOutput& output)
{
    ...
    LDPLAB_PROFILING_START(rtscuda_update_context);
    updateContext(input);
    LDPLAB_PROFILING_STOP(rtscuda_update_context);

    // Execute pipeline
    LDPLAB_LOG_DEBUG("RTSCUDA context %i: Setup ray tracing pipeline",
        m_context->uid);
    LDPLAB_PROFILING_START(rtscuda_setup_pipeline);
    m_context->pipeline->setup();
    LDPLAB_PROFILING_STOP(rtscuda_setup_pipeline);

    LDPLAB_LOG_DEBUG("RTSCUDA context %i: Execute ray tracing pipeline",
        m_context->uid);
