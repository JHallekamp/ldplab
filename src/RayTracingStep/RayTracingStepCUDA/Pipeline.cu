...
ldplab::rtscuda::Pipeline::Pipeline(
...
void ldplab::rtscuda::Pipeline::setup()
{
    LDPLAB_LOG_DEBUG("RTSCUDA context %i: Setup ray tracing pipeline",
        m_context.uid);
    m_initial_stage->setup();
    m_ray_bounding_volume_intersection_test_stage->setup();
}

void ldplab::rtscuda::Pipeline::finalizeOutput(RayTracingStepOutput& output)
{
    for (size_t p = 0; p < m_context.particles.size(); ++p)
    {
        ...
        if (!m_context.flags.return_force_in_particle_coordinate_system)
        {
...
void ldplab::rtscuda::Pipeline::execute(size_t job_id, size_t batch_size)
{
    LDPLAB_ASSERT(job_id < m_buffer_controls.size());
    LDPLAB_LOG_DEBUG("RTSCUDA context %i: Ray tracing pipeline executes "\
        "pipeline instance %i",
    ...
    do
    {
        ...
        LDPLAB_LOG_TRACE("RTSCUDA context %i: Filled batch buffer %i with %i "\
            "initial rays",
            m_context.uid, 
        ...
        {
            LDPLAB_LOG_TRACE("RTSCUDA context %i: Pipeline instance %i executes "\
            ...
            LDPLAB_LOG_TRACE("RTSCUDA context %i: Pipeline instance %i "\
                "finished batch execution",
                m_context.uid, job_id);
        }
    } while(batches_left);

    LDPLAB_LOG_DEBUG("RTSCUDA context %i: Pipeline instance %i executed "\
        "successfully in a total of %i batches",
        m_context.uid, job_id, num_batches);
}

void ldplab::rtscuda::Pipeline::processBatch(
    RayBuffer& buffer, BufferControl& buffer_control)
{
        ...
        m_context.flags.emit_warning_on_maximum_branching_depth_discardment)
    {
        ...
        LDPLAB_LOG_WARNING("RTSCUDA context %i: Pipeline reached max branching "\
