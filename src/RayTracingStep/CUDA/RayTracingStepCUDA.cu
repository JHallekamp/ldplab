	...
	RayTracingStepOutput& output)
{
    ...
    LDPLAB_LOG_INFO("RTSCUDA %i: Ray tracing step starts execution", uid());
    std::chrono::steady_clock::time_point start = std::chrono::steady_clock::now();

    LDPLAB_LOG_DEBUG("RTSCUDA %i: Setup ray tracing pipeline", uid());
    LDPLAB_PROFILING_START(rtscuda_step_setup);
    m_pipeline->stepSetup(input);
    LDPLAB_PROFILING_STOP(rtscuda_step_setup);

    LDPLAB_LOG_DEBUG("RTSCUDA %i: Execute ray tracing pipeline", uid());
    ...
    LDPLAB_LOG_INFO("RTSCUDA %i: Ray tracing step executed after %fs",
        uid(), elapsed_time);
}
