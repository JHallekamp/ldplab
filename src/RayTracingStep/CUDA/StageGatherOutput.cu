...
namespace
{
        ...
        bool particle_space_output)
    {
        ...
        size_t ray_count = num_rays / blockDim.x +
            (num_rays % blockDim.x ? 1 : 0);
        sbuf[force_idx] = Vec3(0);
        sbuf[torque_idx] = Vec3(0);
        unsigned int ri = tid;
    ...
    size_t num_rays)
{
    ...
    const size_t grid_size = smctx.simulationParameter().num_particles;
